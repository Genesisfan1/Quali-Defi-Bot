import { MatrixClient, SimpleFsStorageProvider, AutojoinRoomsMixin, AutojoinUpgradedRoomsMixin, RustSdkCryptoStorageProvider, MatrixAuth } from "matrix-bot-sdk";
...
import { getMostViewed } from "./news.js";
...
let client;
function buildInteractiveMenu(roomId, actionBase) {
...
async function createUnencryptedRoomAndInvite(userId) {
...
async function handleAdminCommand(roomId, body) {
    ...
    switch (parsed.intent) {
        ...
        case Intent.NEWS_TIMEFRAME: {
            ...
            const list = await getMostViewed(tf);
            const actionBase = APP_BASE_URL || `http://localhost:${PORT}`;
        ...
        case Intent.SWAP: {
            ...
            if (!base || !quote || !amount) {
                await client.sendText(roomId, "Usage: !swap <amount> <BASE> to <QUOTE>");
                return;
            }
            const pair = await resolveTokenPair(base, quote);
            if (!pair) {
                await client.sendText(roomId, "Unsupported pair");
                return;
            }
            const req = {
            ...
            if (!quoteRes) {
                await client.sendText(roomId, "No route available");
                return;
            }
        ...
        case Intent.ADJUST_SLIPPAGE: {
            ...
            if (!sess.lastQuote) {
                await client.sendText(roomId, "No active quote");
                return;
            }
            const q = await getQuote({ ...sess.lastQuote.request, slippageBps: Math.round(newSlip * 100) });
            if (!q) {
                await client.sendText(roomId, "Could not refresh quote");
                return;
            }
        ...
        case Intent.ACCEPT: {
            const sess = getSession(roomId);
            if (!sess.lastQuote) {
                await client.sendText(roomId, "No quote to accept");
                return;
            }
            if (!APP_BASE_URL) {
                await client.sendText(roomId, "Signing app URL not configured (APP_BASE_URL). Ask admin to set it.");
                return;
            }
...
export async function onRoomEvent(c, roomId, event) {
    try {
        const type = event["type"];
        const sender = event["sender"];
        const evId = event["event_id"];
        if (DEBUG_LOGS)
            console.log(`[evt] room=${roomId} type=${type} from=${sender} id=${evId}`);
        if (type === "m.room.encrypted") {
            const session = getSession(roomId);
            if (!session["decryptionWarned"]) {
                try {
                    await c.sendText(roomId, "I received an encrypted message but could not decrypt it. Please ensure your room shares keys with unverified devices or invite me to a non-encrypted room.");
                }
                catch { }
...
export async function onRoomMessage(c, roomId, event) {
    try {
        if (DEBUG_LOGS) {
        ...
        switch (parsed.intent) {
          ...
          | <a href="${actionBase3}/act?id=${actBack}">BACK</a>`;
            ...
            case Intent.ADJUST_SLIPPAGE: {
                ...
                if (!q) {
                    await c.sendText(roomId, "Couldn't refresh quote with that slippage.");
                    break;
                }
                setSession(roomId, { ...sess, lastQuote: q, slippagePct: newSlip });
            ...
            case Intent.ACCEPT: {
                const sess = getSession(roomId);
                if (!sess.lastQuote) {
                    await c.sendText(roomId, "No quote to accept. Try a swap first.");
                    break;
                }
                if (USER_SIGNING_MODE || !signer) {
                    if (!APP_BASE_URL) {
                        await c.sendText(roomId, "Signing app URL not configured (APP_BASE_URL).");
                        break;
                    }
                    const q = sess.lastQuote;
                    const u = new URL(APP_BASE_URL);
...
async function seedActionReactions(c, roomId, eventId) {
...
app.post("/admin/command", async (req, reply) => {
    try {
        if (!ADMIN_API_TOKEN)
            return reply.code(403).send({ ok: false, error: "admin disabled" });
        const token = req.headers["x-admin-token"];
        if (!token || token !== ADMIN_API_TOKEN)
            return reply.code(401).send({ ok: false, error: "unauthorized" });
        const { roomId, text } = req.body || {};
        if (!roomId || !text)
            return reply.code(400).send({ ok: false, error: "roomId and text required" });
...
app.get("/act", async (req, reply) => {
    try {
        ...
        try {
            await client.sendText(ticket.roomId, ticket.text);
        }
        catch { }
...
app.post("/admin/onboard", async (req, reply) => {
    try {
        if (!ADMIN_API_TOKEN)
            return reply.code(403).send({ ok: false, error: "admin disabled" });
        const token = req.headers["x-admin-token"];
...
(async () => {
    ...
    if (ACCESS_TOKEN) {
        ...
        try {
            ...
            console.error("Password login failed:", e?.message || e);
            process.exit(1);
        }
    }
    else {
    ...
    try {
        ...
        console.warn("getJoinedRooms failed", e?.message || e);
    }
