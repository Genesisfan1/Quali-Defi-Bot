...
export function registerSigningRoutes(app: ReturnType<typeof Fastify>, rpcUrl: string) {
  app.get('/sign/swap', async (req: any, reply: any) => {
    const q = req.query || {};
    ...
    try {
      ...
      if (gweiMaybe > 1e6) gweiMaybe = gweiMaybe / 1e9; // looked like wei, convert to gwei
...
<body>
  ...
  app.post('/sign/buildSwap', async (req: any, reply: any) => {
    try {
      ...
      const sanitizeTx = (tx: Record<string, any>) => {
        const out: Record<string, any> = {};
        for (const [k, v] of Object.entries(tx)) {
          if (typeof v === 'bigint') out[k] = ethers.toQuantity(v);
          else out[k] = v;
        }
        return out;
      };

      (txRequest as any).chainId = Number(chainId || 1);
      const safeTx = sanitizeTx(txRequest as any);

      // Humanize expected outputs
      let amountOutHuman: string | undefined;
      let minOutHuman: string | undefined;
      try {
