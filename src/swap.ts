...
export type SwapResult = { hash: string };
...
export async function performSwap(
  ...
  const feeTier = Number(quote.raw?.feeTier || 3000) as 500 | 3000 | 10000;
  ...
  if (bestOut === 0n) throw new Error("Invalid quote: amountOut is 0");
...
async function ensureAllowance(
  ...
  const erc20 = new ethers.Contract(token, ERC20_ABI, provider).connect(wallet) as any;
  const current: bigint = await erc20.allowance(wallet.address, spender);
